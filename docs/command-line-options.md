...
- init
- add [--empty][--open][--packages=glob]
- version [--ignore, --snapshot, --packages=glob]
- publish [--otp=code, --tag, --packages=glob]
- status [--since-master --verbose --output=JSON_FILE.json --packages=glob]
- pre [exit|enter {tag}]
...
- `--packages=glob` - packages optionally allows you to pass a custom glob pattern to use to find your projects packages
...
Version has three options, `ignore`, `snapshot` and `packages`:
...
```
changeset version --packages="libs/*"
```

Packages optionally allows you to pass a custom glob pattern to use to find your projects packages.
...
```
status [--verbose] [--output={filePath}] [--since={gitTag}] [--packages={glob}]
```
...
## Custom monorepo setups

Changesets will automatically find all the packages in your project if the projects follows any of the following monorepo configurations:

- root - a single `package.json` file in the root folder
- [yarn](https://classic.yarnpkg.com/lang/en/docs/workspaces/) - `workspaces` configured in `package.json`
- [lerna](https://lerna.js.org/) - a `lerna.json` config file in root
- [pnpm](https://pnpm.io/workspaces) - a `pnpm-workspace.yaml` config file in root
- [bolt](https://github.com/boltpkg/bolt) – `bolt.workspaces` configured in `package.json`

But all projects doesn't follow this structure, e.g. [nx](https://nx.dev/). The changeset cli commands `add`, `version`, `publish` and `status` accepts the `--packages` flag to which you can pass a custom glob which will be used to find your projects packages, instead of the default automatic version.
